...
public class Grid {
    ...
    public boolean addPositionable(Positionable positionable, Vector position) {
        ...
        if (reversedGrid.containsKey(positionable)) {
            if (positionable instanceof Rack) {
                ArrayList<Vector> positions = reversedGrid.get(positionable);
                if(!Vector.isAdjacent(positions.get(positions.size()-1), position)) return false;
     ...
     */
    public ArrayList<Vector> getPositions(Positionable positionable) {
        ArrayList<Vector> list = reversedGrid.get(positionable);
        if(list == null) return null;
        return list;
    }
